...
# Add HomeSection import
content = content.replace("import { api } from '../lib/api';", "import { api, HomeSection } from '../lib/api';")

# Add homeLayout state
state_code = """
  const [homeLayout, setHomeLayout] = useState<HomeSection[]>([]);
  const [isLoadingLayout, setIsLoadingLayout] = useState(true);
"""
content = content.replace("const [productsList, setProductsList] = useState<Product[]>(mockProducts);", state_code + "  const [productsList, setProductsList] = useState<Product[]>(mockProducts);")

# Add api.getHomeLayout to useEffect
fetch_code = """    api.getHomeLayout().then(res => {
      if (active) {
        setHomeLayout(res);
...
"""
content = content.replace("api.getCategories().then", fetch_code + "    api.getCategories().then")


# Now we need to extract sections. I will replace the main return with a modular one.
new_return = """
  // Extract Sections
  const renderHero = () => (
